...
class DocumentRequestController extends Controller
{
    public function store(Request $request, Venue $venue)
    {
        ...
        if ($documentRequest) {
            return back()->with('success', '「' . $venue->name . '」への見学・入園相談はすでに受け付けています。');
        }
        ...
        return back()->with('success', '「' . $venue->name . '」への見学・入園相談を受け付けました。LINEで受付完了のお知らせをお送りします。');
    }
    ...
    public function sendConfirmation(DocumentRequest $documentRequest): void
    {
        ...
        $text = "「{$venue->name}」への見学・入園相談を受け付けました。";
        if ($venue->phone) {
