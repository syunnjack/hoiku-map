...
@section('title', '保育園・幼稚園を投稿する - ' . config('app.name'))
@section('description', '地図をタップして場所を選び、認可保育園・幼稚園の名称・住所などを投稿できます。ログイン不要・匿名で投稿可能です。')

@section('content')
<div class="container my-4">
  <h1 class="h4 mb-3">➕ 保育園・幼稚園を投稿する</h1>
  ...
  <form method="POST" action="{{ route('venues.store') }}" class="bg-light p-3 rounded shadow-sm">
    ...
    <div class="mb-3">
      <label class="form-label">園名 <span class="text-danger">*</span></label>
      <input type="text" name="name" value="{{ old('name') }}" class="form-control" required>
    </div>

    <div class="mb-3">
      <label class="form-label">種別（任意）</label>
    ...
    <button type="submit" class="btn btn-hoiku w-100">登録する</button>
  </form>
</div>
@endsection
